...
uint8_t glProbeCtrl[CY_FX_UVC_MAX_PROBE_SETTING] = {
    ...
    0x00, 0x04, 0x0B, 0x00,     /* Max video frame size in bytes 0x00, 0x48, 0x3F, 0x00*/
        ...
        uint8_t  *buffer)
{
	...
	bufferTemp[0] = (buffer[0]>>4)|0x30;//buffer[1];
	bufferTemp[1] = buffer[0]<<4;//buffer[0];
    CyU3PReturnStatus_t status = CY_U3P_SUCCESS;

	CyU3PSpiSetSsnLine (CyFalse);
	status = CyU3PSpiTransmitWords (bufferTemp, 2);
	if (status != CY_U3P_SUCCESS)
	{
